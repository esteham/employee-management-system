...
const GeneratePayroll = () => {
  ...
  const handleFetchPayrollData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}backend/api/payroll/fetch_employee_payroll.php`,
        {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}backend/api/payroll/generate.php`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleReportView = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}backend/api/payroll/reports.php`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          <Table striped bordered hover responsive className="mt-3">
            <tbody>
              ...
              <tr className="table-primary">
                ...
                <td>
                  <strong>{formatCurrency(payrollData.net_salary)}</strong>
                </td>
              </tr>
            </tbody>
          </Table>
      ...
      {reportData && (
        <div className="mt-5">
          <div className="d-flex justify-content-between align-items-center mb-3">
            ...
            <div>
              <strong>Period: </strong>
              {new Date(formData.year, formData.month - 1).toLocaleDateString(
                "en-US",
            ...
            <Table striped bordered hover responsive>
              ...
              <tbody>
                {reportData.map((row, index) => (
                  <tr
                    key={`${row.id || index}-${formData.month}-${
                      formData.year
                    }`}
                  >
                    ...
                    <td>
                      <strong>{formatCurrency(row.net_salary)}</strong>
                    </td>
                  </tr>
                ))}
              </tbody>
